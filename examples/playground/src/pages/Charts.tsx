...
const categoryKeys = ["ARR", "APP", "APT", "DEP", "REF"]
      ...
      <div className="py-1 px-4 border-y-[0.5px] border-ng-background-500 flex gap-2 items-center justify-between hover:bg-ng-background-300 cursor-pointer">
        ...
        <div className="flex gap-2">
          {chart.is_georeferenced && (
            <Button selected={chartOverlay?.id === chart.id} onClick={() => setChartOverlay(chart)}>
              <IoLayers className="text-gray-25" size={20} />
            </Button>
          )}
          <Button selected={open} onClick={() => setOpen(x => !x)}>
            <LuTableProperties className="text-gray-25" size={20} />
          </Button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className="flex flex-col overflow-auto flex-1 border-[1px] border-ng-background-500 w-full">
        {index
          .filter(chart => chart.category === categoryKeys[category])
          .map(chart => (
      ...
      <div className="page-container flex flex-col items-center gap-3">
        ...
        {icao.length === 4 &&
          (isLoading ? (
            ...
            <AirportPane airport={airport} charts={charts} />
          ) : (
            <span>{icao} has no Charts</span>
          ))}
      </div>
    )
  },
